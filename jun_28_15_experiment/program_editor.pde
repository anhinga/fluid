...
class ProgramEditor {
  ...
  ProgramEditor() {
    ...
    wave_graph_template = new DataFlowGraph(0, 0, 470, 970, true);
    ...
    negation_graph_template = new DataFlowGraph(0, 0, 470, 970, true);
  ...
  void tweak_optionally(DataFlowGraph program) {
    ...
    if (frameCount == 1) {
      println("Adding self-vertex"); 
      DataFlowVertex new_vertex = new DataFlowVertex(program, 10, 525, 450, 450, true); // might want shift to be a bit larger than 0, 0
      VertexDataDFGraph dfgraph_new_vertex = new VertexDataDFGraph();
      new_vertex.set_vertex_data(dfgraph_new_vertex);
      dfgraph_new_vertex.set_df_graph(program);
      program.add_vertex(new_vertex);      
    }

    if (frameCount == 50) { 
      println("Adding wave graph");
      program.add_subgraph(wave_graph_template.limited_deep_copy(510, 25));
    } 
    
    if (frameCount == 100) {
      println("Adding negation graph");
      program.add_subgraph(negation_graph_template.limited_deep_copy(1010, 25));      
    }
    ...
    if (frameCount == 150) {
      ...
      special_insert(main.subgraphs[1].target_nodes[0], main.subgraphs[0].target_nodes[1], 50, 300, true); 
    }
    ...
    if (frameCount > 200 && frameCount <= 300) {
      ...
      sum_of_2_data.interpolation_coef = (frameCount - 200)/100.0;
    }
    
    if (frameCount > 300 && frameCount <= 350) {
      VDID_SumOf2Transform sum_of_2_data = (VDID_SumOf2Transform)main.subgraphs[1].target_nodes[0].vertex_data;
      sum_of_2_data.interpolation_coef = (400-frameCount)/100.0;
    }
